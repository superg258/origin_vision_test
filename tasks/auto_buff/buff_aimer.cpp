  ...
  const bool to_now, double & yaw, double & pitch)
{
  ...
  // 计算目标点的空间坐标
  auto aim_in_world = target.point_buff2world(Eigen::Vector3d(0.0, 0.0, aim_radius_m_));
  ...
  // 计算新的目标点的空间坐标
  aim_in_world = target.point_buff2world(Eigen::Vector3d(0.0, 0.0, aim_radius_m_));
